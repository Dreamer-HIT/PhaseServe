...
from phaseserve.config import (
    ModelConfig,
    ParallelConfig,
    CacheConfig,
)
from phaseserve.request import (
    Request, 
    BatchedRequests,
    MigratingRequest
)
from phaseserve.utils import Counter, cudaMemoryIpcHandle, Stage
from phaseserve.lifetime import LifetimeEvent, LifetimeEventType
from phaseserve.tokenizer import get_tokenizer
from phaseserve.block_manager import BlockManager
from phaseserve.worker import ParaWorker
from phaseserve.context_stage_scheduler import ContextStageSchedConfig, ContextStageScheduler, get_context_stage_scheduler
from phaseserve.decoding_stage_scheduler import DecodingStageSchedConfig, DecodingStageScheduler, get_decoding_stage_scheduler
...
class ContextStageLLMEngine(SingleStageLLMEngine):
    ...
    async def _step(self):
        ...
        # # 调试输出：每次调度出的batch内容
        # if hasattr(batched_requests, "requests"):
        #     print(f"[调试] 当前调度batch: {[r.request_id for r in batched_requests.requests]}")
...
#                 # 调试输出：输出已完成batch及等待队列信息
#                 print(f"[调试] finished_batch.requests: {[r.request_id for r in finished_batch.requests]}")
#                 if hasattr(self.scheduler, "waiting_queue"):
#                     print(f"[调试] scheduler.waiting_queue长度: {len(self.scheduler.waiting_queue)}")
#                 if hasattr(self, "batches_in_pipeline"):
#                     print(f"[调试] batches_in_pipeline长度: {len(self.batches_in_pipeline)}")

#                 # 修改点：若flag为False，则context引擎自行处理请求至结束，不转发decode
#                 flag = False
#                 for request in finished_batch.requests:
#                     if not request.is_finished:
#                         if flag:
...
#                                 self.parallel_config,
#                             )
#                             self.bridge_queue.put_nowait(migrating_req)  # 解耦模式，推送给bridge队列
#                         else:
#                             print("非解耦模式，本引擎内批处理同步步进推理直到完成")
#                             # 批处理同步步进推理（参考decode引擎）
#                             max_steps = 128  # 最大生成步数
#                             eos_token_id = getattr(self.tokenizer, "eos_token_id", 50118)
#                             pad_token_id = getattr(self.tokenizer, "pad_token_id", 0)
#                             unk_token_id = getattr(self.tokenizer, "unk_token_id", 0)
#                             special_tokens = {eos_token_id, pad_token_id, unk_token_id, 50118, 2, 0}

#                             # 初始化所有未完成请求
#                             active_requests = [request for request in finished_batch.requests if not request.is_finished]
#                             # 为每个请求初始化生成token序列
...
#                                 # 构造batch输入：每个请求当前token序列
#                                 input_token_ids_list = []
#                                 input_lengths = []
#                                 for req in active_requests:
#                                     input_token_ids_list.append(req.prompt_token_ids + req.generated_token_ids)
#                                     input_lengths.append(len(input_token_ids_list[-1]) - 1 if len(input_token_ids_list[-1]) > 0 else 0)
#                                 # 批量推理
#                                 remote_calls = self._remote_call_all_workers_async(
#                                     "step",
#                                     [req.request_id for req in active_requests],
#                                     input_token_ids_list,
#                                     input_lengths,
#                                     self.block_manager.get_partial_block_table([req.request_id for req in active_requests])
#                                 )
#                                 pp_size = self.parallel_config.pipeline_parallel_size
#                                 tp_size = self.parallel_config.tensor_parallel_size
#                                 # 假设只有一个worker（单GPU），直接取remote_calls[0]
#                                 step_results = await remote_calls[0]
#                                 # 若返回单token，则转为list
...
#                                     step_result = step_results[idx] if idx < len(step_results) else step_results[-1]
#                                     try:
#                                         token = self.tokenizer.decode(step_result)
#                                     except Exception as e:
#                                         print(f"(context, batch-standalone) Warning: Cannot decode token with id {step_result}. Error: {e}")
#                                         token = ""
#                                     req.generated_tokens.append(token)
#                                     req.generated_token_ids.append(step_result)
#                                     # 终止条件
#                                     if step_result in special_tokens or len(req.generated_token_ids) >= max_steps:
#                                         req.is_finished = True
#                                         finished_indices.append(idx)
#                                     # 调试输出
#                                     print(f"[调试] request {req.request_id} 步{step_count} 输入长度: {len(input_token_ids_list[idx])} 新token_id: {step_result} 当前gen_token_ids: {req.generated_token_ids} is_finished: {req.is_finished}")
#                                     # 回调
#                                     step_output = StepOutput(req, token, step_result)
#                                     self.engine_on_new_lifetime_event_callback(
#                                         req.request_id,
#                                         LifetimeEvent(LifetimeEventType.ContextEnd)
#                                     )
...
#                                 if req.request_id in self.block_manager.block_table:

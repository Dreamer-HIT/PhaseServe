...
Default path where facebook/opt-125m is saved: 
    ~/.cache/phaseserve/models--facebook--opt-125m/
"""
import filelock
import os
import torch

from phaseserve.config import ModelConfig
...
default_cache = os.path.join(os.path.expanduser("~"), ".cache", "phaseserve")
PHASESERVE_CACHE = os.getenv("PHASESERVE_CACHE", default_cache)
